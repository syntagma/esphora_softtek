		...
		if ($logo_name) //error checking just in case, by OpenCRM
{
			...
			$stretchx = $location [2];
			$stretchy = $location [3];
			$this->Image ( $logo_name, $x1, $y1, $stretchx, $stretchy );
		}
	}
	...
	function title($label, $total, $position) {
		...
		$this->RoundedRect ( $r1 - 16, $y1 - 1, $r2-$r1, $y2 + 1, 2.5, 'DF' );
		$this->SetXY ( $r1 + 15, $y1 + 1 );
	...
	// Data block, non-wrapped
	function addTitleBlock($title, $text, $positions, $labelw) {
		$r1 = $positions [0];
		$y1 = $positions [1];
		$this->SetXY ( $r1, $y1 );
		$this->SetFont ( "Helvetica", "B", 10 );
		$this->Cell ( $labelw, 4, $title );
		$this->SetXY ( $r1+$labelw, $y1 );
		$this->SetFont ( "Helvetica", "", 10 );
		$this->Cell ( $positions [2], 4, $text );
	}
	...
	function addTotalsRec($names, $totals, $positions) {
		...
		$this->Line ( $r1 + 27, $y1, $r1 + 27, $y2 ); // Importe Neto Gravado
		$this->Line ( $r1 + 43, $y1, $r1 + 43, $y2 ); // IVA 27%
		$this->Line ( $r1 + 66, $y1, $r1 + 66, $y2 ); // IVA 21%
		$this->Line ( $r1 + 89, $y1, $r1 + 89, $y2 ); // IVA 10.5%
		$this->Line ( $r1 + 112, $y1, $r1 + 112, $y2 ); // IVA 0%		
		$this->Line ( $r1 + 135, $y1, $r1 + 135, $y2 ); // Otros Tributos
		$this->Line ( $r1 + 158, $y1, $r1 + 158, $y2 ); // Total		
		...
		$this->SetX ( $r1 + 89 );
		$this->Cell ( 10, 4, $names [4] );
		$this->SetX ( $r1 + 112 );
		$this->Cell ( 10, 4, $names [5] );
		$this->SetX ( $r1 + 135 );
		$this->Cell ( 10, 4, $names [6] );
		...
		$this->SetXY ( $r1 + 89, $y1 + 5 );
		$this->Cell ( 10, 4, $totals [4] );
		$this->SetXY ( $r1 + 112, $y1 + 5 );
		$this->Cell ( 10, 4, $totals [5] );
		$this->SetXY ( $r1 + 135, $y1 + 5 );
		$this->Cell ( 10, 4, $totals [6] );
							
		$this->SetFont ( "helvetica", "B", 6 );
		$this->SetXY ( $r1 + 160, $y2 - 8 );
		$this->SetFont ( "Helvetica", "", 10 );
	}

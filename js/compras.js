...
function valida_compra() {
	...
	if (_modo == 'E') {
		...
		if (_modo == 'M') {
			res = required('cotizacion') && numeric('cotizacion') && res;
		}
		
		res = required('cae') && res;
...
function agregaLinea() {
	...
	tagDiv.id = "linea"+lineaActual;
		...
		txt += '<table>';
			txt += '<tr>';
				txt += '<td valign=top><textarea name="txtConcepto'+lineaActual+'" id="txtConcepto'+lineaActual+'" cols=23 rows=3></textarea></td>';
...
function buscaProveedor(proveedor, divId, btnObj) {
	if (proveedor == "") {
		...
		new Ajax.Updater (
			divId,
			'busquedaProveedores.php',
			{
				method: 'get',
				parameters: '?patron='+proveedor,
...
function graba_compra () {
	if (required("cae")) {
		document.forms['frmAltaCompra'].submit();
	}
}
